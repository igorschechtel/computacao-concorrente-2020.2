...
// variáveis globais
int NUM_THREADS, ARRAY_LENGTH, AMOUNT_PER_THREAD, OFFSET;
int* array;
...
int main(int argc, char *argv[]) {
  ...
  OFFSET = ARRAY_LENGTH % NUM_THREADS;
...
void* threadMergeSort(void* tid) {
  ...
  if (threadId == NUM_THREADS - 1) end += OFFSET;
...
void mergeThreadArraySections(int* array, int amount, int section) {
  ...
  if (amount / 2 >= 1) {
    mergeThreadArraySections(array, amount / 2, section * 2);
  }
}
